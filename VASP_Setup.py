    ...
    def make_poscar_rescaled(self, cell_rescale_dict, is_strained=False):
        ...
        if line_count == 5:
            for element in element_names:
                poscar_scaled.write(str(element)+" ")
            poscar_scaled.write("\n")
            line_count += 1
        if line_count == 6:
        ...
        for key in surface_trimming_dict.keys():
            ...
            if surface_trimming_dict[key] > 0:
                ...
                while surface_layer_count < surface_trimming_dict[key]:
                    ...
                    element_names = PoscarAnalyzer(poscar=self.poscar).get_element_names()
                    atom_amounts = PoscarAnalyzer(poscar=self.poscar).get_atom_amounts()
                    ...
                    line_element_identity = dict()
                    count = 0
                    for i in range(len(atom_amounts)):
                        atom_coord_lines = range(count+line_where_coords_start+1,count+line_where_coords_start+atom_amounts[i]+1)
