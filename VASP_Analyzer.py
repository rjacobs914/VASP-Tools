...
class PoscarAnalyzer(object):
    ...
    def get_element_list(self):
        #composition_dict = self.get_composition_dict()
        atom_amounts = self.get_atom_amounts()
        element_names = self.get_element_names()
        element_list = list()
        total_atom_count = 0
        for element_name, element_amount in zip(element_names, atom_amounts):
            for atom in range(element_amount):
...
class OutcarAnalyzer(object):
    ...
    def get_magnetization_per_atom(self):
        mag_data_raw = []; mag_data_dict = {}; mag_data_dict_element_labels={}
        line_counting = False
        total_atoms = PoscarAnalyzer().get_total_atoms()
        outcar = open(str(self.outcar), "r")
        element_list = PoscarAnalyzer().get_element_list()
        ...
        for atom, mag in mag_data_dict.items():
            mag_data_dict_element_labels[str(element_list[int(atom)-1])+str(atom)]= mag                

        return mag_data_dict_element_labels

    def get_charge_per_atom(self):
        chg_data_raw = []; chg_data_dict = {}
        line_counting = False
        total_atoms = PoscarAnalyzer().get_total_atoms()
        outcar = open(str(self.outcar), "r")
        for line in outcar.readlines():
            if "total charge" in line:
                ...
                line_counting = True
            if line_counting == bool(True) and line_count < total_atoms+5:
                chg_data_raw.append(line.strip())
                line_count += 1
        # Only report the last batch of magnetization data, which is for the last ionic iteration
        max_charge_index = 0
        for index, entry in enumerate(chg_data_raw):
            if "total charge" in entry:
                if index > max_charge_index:
                    max_charge_index = index
        for index, entry in enumerate(chg_data_raw):
            if index >= max_charge_index+4 and index < max_charge_index+total_atoms+4:
                chg_data_dict[int(entry.split()[0])] = float(entry.split()[-1])
        return chg_data_dict
    ...
    def get_charge_for_element_type(self, element, write_to_file=False):
        chg_data_dict = self.get_charge_per_atom()
        pa = PoscarAnalyzer()
        element_names = pa.get_element_names()
        atom_amounts = pa.get_atom_amounts()
        composition = pa.get_composition_dict(write_to_file=False)
        element_label_dict = {}
        element_chg_dict = dict()
        element_chg_dict_norm = dict()
        ...
        # Go through mag_data_dict and add magnetization of element type arg
        for value1, value2 in zip(element_label_dict.values(), chg_data_dict.values()):
            if value1 == element:
                element_chg += float(value2)

        # Normalize element_mag by the number of atoms of that type
        for key, value in composition.items():
            if key == element:
                number_of_atoms_for_element = value
        element_chg = element_chg/number_of_atoms_for_element

        if write_to_file == bool(True):
            filestr = element+"_element_charge.txt"
            file = open(filestr, "w")
            file.write(str(element_chg))
            file.close()
